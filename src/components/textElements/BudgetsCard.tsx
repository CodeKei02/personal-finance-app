...
import {  updateBudgetItem, deleteBudgetItem } from '../../store/slices/budgetsSlice';
...
export const BudgetsCard: React.FC<FormBudgets> = ({budgets, validationSchema}) => {
    const { transactions } = useSelector((state: RootState) => state.transaction)
    const { selectedItem, items } = useSelector((state: RootState) => state.budget);
        ...
        <ContainerCard className='card-container'>
            {items.map((budget: any) => (
                <Card key={budget.id} id={budget.id}>
                    ...
                    <div>
                        <div style={{ backgroundColor: budget.theme }} className='barra'></div>
                    </div>
                    <div className='barra-container'>
                        <div className='barra-horizontal' style={{ backgroundColor: budget.theme, width: `${Math.abs(budget.maximum)}px` }}></div>
                    </div>
                    <div className='card-content'>
                        <div className='card-text'>
                            <div className='barra-vertical' style={{ backgroundColor: budget.theme }}></div>
                            <div>
                                <p>Spent</p>
                        ...
                        <div className='card-text'>
                            ...
                            <div>
                                <p>Free</p>
                                <strong>${budget.maximum - totalSpentByCategory(budget)}</strong>
                            </div>
                    ...
                    <ListContainer>
                        ...
                        <ul>
                            {transactions.filter((transaction: any) => transaction.category == budget.category).slice(0, 3).map((transaction: any) => (
